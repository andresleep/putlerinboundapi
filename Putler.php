...
class Putler
{
	...
	private function currencyExchange($currency) {
		$string = file_get_contents('https://www.apilayer.net/api/live?access_key=075bf6009fe4355529edc0a9edf9fe7f');
		$json_a = json_decode($string, true);
		return $json_a['quotes'][$currency];
	}

	public function putlerValidations() {
	...
	public function putlerPost($oid, $transaction_fee) {
		...
		if ($validation->ACK == "Success") {
			if(!empty($oid) && is_numeric($oid)) {
				...
				$fee   	= round((($transaction_fee/100)/$this->currencyExchange('USDMXN')), 2);
